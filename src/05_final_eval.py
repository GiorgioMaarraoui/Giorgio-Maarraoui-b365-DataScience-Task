...
In this script, I produce the following 7 artifacts:
  ...
  * artifacts/eval/final_headline.csv         all variants on the test set
...
The headline model is picked by CRPS on the test set (the proper distributional score), with ties broken by MAE.
"""
...
# Choose final by CRPS (lower better, NaN dropped).
crps_sorted = comp[comp["crps"].notna()].sort_values("crps")
best = crps_sorted.iloc[0]
print(f"Best by CRPS: {best['variant']} (CRPS={best['crps']:.4f})")
...
# ----- over/under at lines using the predictions.parquet's NB -----
...
ax.set_title("Final model: predicted vs empirical over-rate by line")
ax.legend()
...
ax.set_xlabel("PIT"); ax.set_title("Final model: randomised PIT"); ax.legend()
